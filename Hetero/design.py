...
class _af_design:
  ...
  def _recycle(self, model_params, key, params=None, opt=None):
    ...
    if self.args["recycle_mode"] == "average":
      ...
      #L = self._inputs2["residue_index"].shape[-1]
      #self._inputs2.update({'init_pos': np.zeros([1, L, 37, 3]),
      #                     'init_msa_first_row': np.zeros([1, L, 256]),
       #                    'init_pair': np.zeros([1, L, L, 128])})
      ...
      for r in range(opt["recycles"]+1):
        ...
        if self.protocol == "hetero":
          (loss,outs),_grad = self._grad_fn(params, model_params, self._inputs, _key, _opt) #self._grad_fn(params, model_params, self._inputs,self._inputs1,self._inputs2, _key, _opt)
        if self.protocol != "hetero":
          (loss,outs),_grad = self._grad_fn(params, model_params, self._inputs, _key, _opt) #self._grad_fn(params, model_params, self._inputs,self._inputs2, _key, _opt)
        grad.append(_grad)
  ...
  def _step(self, weights=None, lr_scale=1.0, **kwargs):
    ...
    #wandb.log({los: self._losses[key]})
