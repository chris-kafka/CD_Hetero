...
class _af_loss:
  def _get_fn(self):
    ...
    def mod(params, model_params, inputs,  key, opt): #, inputs1 ,inputs2 ,
      ...
      if self.protocol in ["fixbb","hallucination","hetero"] and self._copies > 1:
      ...
      #outputs1 = self._runner.apply(model_params, key, inputs1)
      #outputs2 = self._runner.apply(model_params, key, inputs2)
        ...
        if self.DNA != None:
          ...
          with open(path) as f:
            ...
            for i, line in enumerate(reader):
              # print(line[0].split()[6])
              # data.append('<|endoftext|>')

              dt = line[0].split()
              if dt[2] == 'P' or dt[2] == 'C5' or dt[2] == 'C2':
                dx.append(dt[6])
                dy.append(dt[7])
                dz.append(dt[8])
          ...
          l = len(dz)
          data = np.zeros((l, 3))
          data[:, 0] = np.asarray(dx)
          data[:, 1] = np.asarray(dy)
          data[:, 2] = np.asarray(dz)
          data = data - data.mean(-2,keepdims=True)

          pos = opt["pos"]
          pos_r = opt["pos_r"]
          '''''
          pos_full = jnp.asarray(jnp.arange(len(outputs["structure_module"]["final_atom_positions"][:])))

          pos_id = jnp.isin(pos_full, pos,invert=True)
          pos_r = jnp.zeros(len(outputs["structure_module"]["final_atom_positions"][:]) - len(pos))
          k2 = 0
          for i2 in range(len(pos_id)):

            pos_r = pos_r.at[k2].set(int(lax.cond(pos_id[i2], lambda x: x,pos_full[i2])))
            k2 = k2 + 1
            #if pos_id[i2] == False:
             #pos_r = pos_r.at[k2].set(int(pos_full[i2]))

          # print()
          '''''

          coords = outputs["structure_module"]["final_atom_positions"][pos_r, 1, :]

          true = self._batch["all_atom_positions"][:,1,:]
          pred = outputs["structure_module"]["final_atom_positions"][pos,1,:]
          p = true- true.mean(-2,keepdims=True)
          q = pred -pred.mean(-2,keepdims=True)
          r= jnp.array(data, float)



          r = r @ _np_kabsch(p, q, use_jax=True)
          print(r)
          d =jnp.array(coords,float)
          da = jnp.zeros(len(r[:, 0]) * len(d[:, 0]))
          k1 = 0

          for i1 in range(0,len(r[:, 0]),5):
            for j1 in range(0,len(d[:, 0])):


              #da = da.at[k1].set(jax.nn.relu(-jnp.sqrt(jnp.square(jnp.subtract(r[i1, 0], d[j1, 0])) + jnp.square(jnp.subtract(r[i1, 1], d[j1, 1])) +jnp.square(jnp.subtract(r[i1, 2], d[j1, 2]))))+2)
              da = da.at[k1].set(100*jax.nn.relu(-jnp.linalg.norm(r[i1, :]- d[j1, :]) + 1))
              # print(da[k1])
              k1 = k1 + 1



          losses["rep"] = jnp.sum(da)

      if self.protocol == "hallucination":
        if self._shape ==True:
          positions = outputs["structure_module"]["final_atom_positions"][:, 1, :]

          data = np.zeros((self._len, 3))
          d_a = jnp.zeros(50)

          for i in range(self._len):
            x = 20 * np.random.ranf() - 10
            if x >= 6:
              y = 20 * np.random.ranf() - 10
            else:
              y = 4 * np.random.ranf() - 2
            z = 2 * np.random.ranf() - 2
            data[i, 0] = x
            data[i, 1] = y
            data[i, 2] = z
            d_a = d_a.at[i].set(_np_rmsd(data, positions))
            losses["shape"] = d_a.sum()/50
      ...
      if self.protocol != "hetero":
        coords = outputs["structure_module"]["final_atom_positions"][:, 1, :]


        c_o_m = np.mean(coords, 0)
        rog = jnp.mean(jnp.power((coords - c_o_m), 2))
        sq_rog = jnp.sqrt(rog)

        rog_fin = jax.nn.relu(jnp.mean(sq_rog)-15)

        rg = jax.nn.relu(jnp.mean(sq_rog-5))
      ...
      if self.protocol == "hetero3":
        coords1 = outputs["structure_module"]["final_atom_positions"][:self._len1, 1, :]
        coords2 = outputs["structure_module"]["final_atom_positions"][self._len1:self._len2 + self._len1, 1, :]
        coords3 = outputs["structure_module"]["final_atom_positions"][self._len2 + self._len1:, 1, :]

        c_o_m1 = np.mean(coords1, 0)
        c_o_m2 = np.mean(coords2, 0)
        c_o_m3 = np.mean(coords3, 0)

        rog1 = jnp.mean(jnp.power((coords1 - c_o_m1), 2))
        rog2 = jnp.mean(jnp.power((coords2 - c_o_m2), 2))
        rog3 = jnp.mean(jnp.power((coords3 - c_o_m3), 2))

        sq_rog1 = jnp.sqrt(rog1)
        sq_rog2 = jnp.sqrt(rog2)
        sq_rog3 = jnp.sqrt(rog3)

        rog_fin = jax.nn.relu(jnp.mean(sq_rog1) - self._rog) + jax.nn.relu(jnp.mean(sq_rog2) - self._rog) + jax.nn.relu(
          jnp.mean(sq_rog3) - self._rog)
      ...
      if self.protocol =="hetero":
        L = self._len1
        coords1 = outputs["structure_module"]["final_atom_positions"][:L, 1,:]
        coords2 = outputs["structure_module"]["final_atom_positions"][L:, 1, :]
        ...
        rog_fin =jax.nn.relu(jnp.mean(sq_rog1)-self._rog)+jax.nn.relu(jnp.mean(sq_rog2)-self._rog)
  ...
  def _get_Bondloss(self, outputs):
  ...
  def _get_pairwise_loss(self, outputs, opt, offset=None):
    ...
    def get_helix_loss(dgram,c,offset=None):
      '''helix bias loss'''
      x = get_pw_con_loss(dgram, 17.0, binary=False, entropy=c["entropy"])
      offset = None
      if offset is None:
        return jnp.diagonal(x,6).mean()
      else:
        mask = offset == 3
    ...
    if self.protocol in  ["binder",'hetero'] or self._copies > 1:
