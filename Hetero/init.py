...
class _af_init:
  ...
  def _prep_hallucination(self, length=100,shape=False, copies=1,path="/home/gpuser/AF2/V3", **kwargs):
    '''prep inputs for hallucination'''
    self._len = length
    self._inputs = self._prep_features(length * copies)
    self._inputs2 = self._prep_features(length * copies*2)
    self._copies = copies
    self._path = path
    self._shape = shape
    # set weights
    self._default_weights.update({"con":1.0, "rog":1.0, "bh":0.01,'shape':1.0})
    if copies > 1:
      self._inputs["residue_index"] = self.repeat_idx(np.arange(length), copies)[None]
      self._default_weights.update({"i_pae":0.01, "i_con":0.1, "rog":1.0})

    self.restart(set_defaults=True, **kwargs)



  def _prep_hetero(self, length_1=100, length_2=10, copies=1,rog=20,**kwargs):
    '''prep inputs for hallucination'''
    ...
    self._len2= length_2
    #self._len3= length_3
    self._inputs = self._prep_features(length_1 + length_2) #+length_3
    self._inputs1 = self._prep_features(length_1)  # +length_3
    self._inputs2 = self._prep_features(length_2)  # +length_3
    self._copies = copies
    self._rog = rog
    ...
    ind = np.zeros(length_1 + length_2)
    ind[0:length_1] = np.arange(length_1)
    ind[length_1:] = np.arange(length_2) + 50 + length_1

    ind1 = np.arange(length_1)
    ind2 = np.arange(length_2)
    #ind[length_1:length_2+length_1] = np.arange(length_2) + 50 + length_1
    #ind[length_2 +length_1:length_2 + length_1+length_3] = np.arange(length_3) + 100 + length_1+ length_2
    ...
    #print(self._inputs["residue_index"])

    self.restart(set_defaults=True, **kwargs)

  def _prep_partial(self, pdb_filename, dna_pathname=None, chain=None, pos=None, length=None,
    ...
    self._copies = 1
    if dna_pathname != None:
      self.DNA = dna_pathname
    else:
      self.DNA = None
    ...
    pos_full = np.asarray(np.arange(length))

    pos_id = np.isin(pos_full, pos)
    pos_r = np.zeros(length - len(pos))
    k2 = 0
    for i2 in range(length-len(pos)):

      if pos_id[i2] == False:
        pos_r[k2] = pos_full[i2]
        k2 = k2 + 1
    ...
    self._default_opt["pos_r"] = np.ndarray.astype(pos_r,int)
    ...
    weights = {"dgram_cce":1.0,"con":1.0, "fape":0.0, "rmsd":0.0,"6D":0.0,"rep":1.0}
